...
class TestOrderCompleteWorkflow(TestCase):
    ...
    def test_complete_order_workflow(self):
        ...
        # Use localtime + offset to avoid clean_order_day rejecting the date
        future_time = (timezone.localtime() + timedelta(minutes=5)).strftime('%Y-%m-%dT%H:%M')
        
        # 2. Create Order
        form_data = {
            'order_day': future_time,
            'order_name': 'WF Test Order',
        ...
        # 3. Should redirect on success
        self.assertRedirects(response, reverse('orders:order-list'))
        
        order = orders.objects.get(order_name='WF Test Order')
        ...
        # 12. Check if restore StockMovement records were created
