...
class TestDateRangeForm(TestCase):
    ...
    def test_date_range_form_widget_type(self):
        """DateRangeForm widget type test"""
        form = DateRangeForm()
        
        # Check widget types - both should be DateInput
        start_date_widget = form.fields['start_date'].widget
        end_date_widget = form.fields['end_date'].widget
        
        from django.forms import DateInput
        self.assertIsInstance(start_date_widget, DateInput)
        self.assertIsInstance(end_date_widget, DateInput)
        
        # Check widget attrs (id is always set; type may be set by DateInput)
        self.assertEqual(start_date_widget.attrs.get('id'), 'start_date')
        self.assertEqual(end_date_widget.attrs.get('id'), 'end_date')
        if 'type' in start_date_widget.attrs:
            self.assertEqual(start_date_widget.attrs['type'], 'text')
        if 'type' in end_date_widget.attrs:
            self.assertEqual(end_date_widget.attrs['type'], 'text')
